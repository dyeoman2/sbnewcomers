...
function authenticate(callback) {
  ...
  client.post(
    options.tokenHost + options.tokenEndpoint,
    args,
    function (data, response) {
      if (data && data.error) {
        ...
        tokenExpiresAt.setSeconds(
          tokenExpiresAt.getSeconds() + data.expires_in
        );
      }
...
function wrapMethods() {
  ...
  _.each(client.methods, function (method, methodName) {
    client.methods[methodName] = _.wrap(
      method,
      function (originalMethod, args, callback) {
        if (!args) args = {};
...
function registerClientMethods() {
  ...
  client.registerMethod(
    "listContacts",
    baseURL + "/accounts/${accountId}/contacts",
    "GET"
  );
  client.registerMethod(
    "listContact",
  ...
  client.registerMethod(
    "listEvents",
    baseURL + "/accounts/${accountId}/events",
    "GET"
  );
  client.registerMethod(
    "listEvent",
    baseURL + "/accounts/${accountId}/events/${eventId}",
    "GET"
  );
...
exports.init = function (_options) {
  ...
  if (missingOptions.length) {
    throw new Error(
      "The following options are required: " + missingOptions.join(", ")
    );
  }
