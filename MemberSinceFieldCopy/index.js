  ...
  apiClient.methods.updateContact(
    ...
    function (contactDataUpd, response) {
      if (!_.isNil(contactDataUpd) && !_.isNil(contactDataUpd.Id)) {
        processed++;
        log.trace(
          "%d >>> Member since readonly field set for %s %s (ID: %s | status: %s | joined: %s)",
          index + 1,
        ...
        const msg = util.format(
          "%d >>> Failed to copy member since field for contact ID %s",
          index + 1,
          contact.args.data.Id
        );
        log.error(msg);
...
const processContacts = function (contacts, action) {
  ...
  for (var m = 0; m < contacts.length; m++) {
    ...
    const MemberSinceReadOnlySysCode = member.FieldValues.filter(function (
      field
    ) {
