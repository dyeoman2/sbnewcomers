...
router.post("/", cors(corsOptions), function (req, res, next) {
        ...
        switch (contactData.State) {
          ...
          case "Complete":
            ...
            if (!_.isNil(contactData.Contacts)) {
              ...
              resId = contactData.ResultId;
              log.info(
                "Request complete (result ID: %s) -- retrieving contacts ...",
                resId
              );
  ...
  function getInvoice(attendee, index, callback) {
    ...
    if (regType === "Board Members") {
      ...
      if (
        attendee.RegistrationFee > 0 &&
        attendee.PaidSum === attendee.RegistrationFee
      ) {
      ...
      apiClient.methods.listInvoice(
        ...
        function (invoiceData, invResp) {
          if (!_.isNil(invoiceData) && !_.isNil(invoiceData.DocumentNumber)) {
            ...
            const msg = util.format(
              "%d) >>> Failed to get invoice info for %s (ID: %s)",
              index + 1,
              attendee.DisplayName,
              attendee.Id
            );
    ...
    apiClient.methods.listEventRegs(args, function (eventData, response) {
      if (!_.isNil(eventData) && _.isArray(eventData) && eventData.length > 0) {
        ...
        if (_.isArray(attendees) && attendees.length === eventData.length) {
          ...
          const memberArgs = {
            ...
            parameters: {
              ...
              $fields:
                "'First name','Last name','Membership status','Member since','New member',Member since readonly','Membership level','Role','Renewal due'",
            },
          };
        ...
        errorMsg = util.format(
          "Invalid eventId parameter (%s)",
          args.parameters.eventId
        );
        res.render("event", {
          title: "SBNC Event Registration Report",
          error: errorMsg,
  ...
  function renderHtml(attendees, eventData) {
    ...
    for (var i = 0; i < attendees.length; i++) {
      ...
      var regDate = new Date(attendee.RegistrationDate).toLocaleTimeString(
        "en-US",
        dtf
      );
      ...
      if (!_.isNil(renewDate) && renewDate.length > 0) {
        attendee.renewDate = new Date(renewDate[0].Value).toLocaleDateString(
          "en-US",
          df
        );
      } else {
      ...
      if (regType === "Board Members") {
        ...
        if (attendee.OnWaitlist == true) {
          ...
          if (
            null == attendee.invoiceNumber ||
            undefined == attendee.invoiceNumber
          ) {
            paidStatus = "Free";
          } else {
            paidStatus = "Due";
