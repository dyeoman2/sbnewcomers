...
const processContact = function (contact, index, callback) {
  log.trace(
    '%d >>> Processing contact ID %s with action %',
    index + 1,
  ...
  apiClient.methods.updateContact(contact.args, function (contactDataUpd, response) {
    if (!_.isNil(contactDataUpd) && !_.isNil(contactDataUpd.Id)) {
      ...
      const msg = util.format(
        '%d >>> Failed to %s newbie update flag for contact ID %s',
        index + 1,
    ...
    switch (action) {
      case 'setNewbieFlag':
        ...
        if (_.isNil(newbieFlag) || newbieFlag.Label == 'No') {
          ...
          log.trace(
            "Newbie status for %s %s (ID: %s | status: %s | joined: %s) already set to '%s' on %s",
            newbie.FirstName,
            newbie.LastName,
            newbie.Id,
            newbie.Status,
      ...
      case 'clearNewbieFlag':
        ...
        if (!_.isNil(newbieFlag) && newbieFlag.Label == 'Yes') {
          ...
          log.debug(
            ...
            newbie.Status,
            memberSince,
            _.isNil(newbieFlag) || _.isNil(newbieFlag.Label) ? 'NULL' : newbieFlag.Label,
            _.isNil(newbieStatusUpd) ? '' : newbieStatusUpd
          );
        }
          ...
          Message: {
            Body: {
              Html: {
                Charset: 'UTF-8',
                Data: util.format(
                  '%s completed for %d member%s with %d error%s',
                  action,
                  processed,
                  processed > 1 ? 's' : '',
                  errors,
