...
function exportEvents(auth) {
  ...
  apiClient.methods.listEvents(eventArgs, function (eventData, eventResp) {
    if (
      !_.isNil(eventData) &&
      !_.isNil(eventData.Events) &&
      _.isArray(eventData.Events) &&
      ...
      for (var i = 0; i < events.length; i++) {
        ...
        if (
          !_.isNil(event.AccessLevel) &&
          (event.AccessLevel === "Public" || event.AccessLevel === "Restricted")
        ) {
          var rule = null;
          if (
            !_.isNil(event.Sessions) &&
            _.isArray(event.Sessions) &&
            event.Sessions.length > 0
          ) {
            const msg = util.format(
              "Event %d is recurring %d times",
              eventId,
          ...
          if (
            !_.isNil(gEvent) &&
            (gEvent.summary === eventName || gEvent.summary.includes(eventId))
          ) {
            ...
            if (
              ...
              if (
                ...
                gEvent.start.dateTime === event.StartDate &&
                gEvent.end.dateTime != event.EndDate &&
                gEvent.recurrence[0] === rule
              ) {
                const msg = util.format(
                  "Skipping unchanged recurring event %s",
                  eventName
                );
                log.info(msg);
              } else {
            ...
            gEvent = {
              ...
              description: util.format(
                "https://www.sbnewcomers.org/event-%d",
                eventId
              ),
          ...
          const msg = util.format(
            "Ignoring event %d with access level '%s'",
            eventId,
            event.AccessLevel
          );
          log.trace(msg);
      ...
      if (eventsAdded.length + eventsUpdated.length + errors > 0) {
        var htmlMsg =
          util.format(
            "<p>%d event%s created, %d event%s updated, and %d event%s deleted with %d error%s</p>",
            eventsAdded.length,
            eventsAdded.length === 1 ? "" : "s",
            eventsUpdated.length,
            eventsUpdated.length === 1 ? "" : "s",
            eventsDeleted.length,
            eventsDeleted.length === 1 ? "" : "s",
            errors,
            errors === 1 ? "" : "s"
          ) + "<ul>";
        ...
        var textMsg = util.format(
          "%d event%s created, %d event%s updated, and %d event%s deleted with %d error%s",
          eventsAdded.length,
          eventsAdded.length === 1 ? "" : "s",
          eventsUpdated.length,
          eventsUpdated.length === 1 ? "" : "s",
  ...
  try {
    ...
    apiClient.methods.listEvent(eventArgs, function (eventData, eventResp) {
      if (eventResp.statusCode == 404) {
        ...
        log.trace(
          "%d - %s: Event %s was deleted from WildApricot -- deleting from Google",
          eventResp.statusCode,
          eventResp.statusMessage,
          event.summary
        );
    ...
    if (diff < 1) {
      ...
      if (diff < 1) {
        ...
        const rule =
          ...
          sessions[sessions.length - 1].StartDate.substring(0, 19)
            .replace(/\-/g, "")
            .replace(/\:/g, "");
        return rule + "Z";
      }
...
function listEvents(calendar) {
  ...
  calendar.events.list(
    ...
    (err, res) => {
      ...
      if (results.length < 1) {
        const msg = util.format(
          "No events found for time period %s - today",
          startDate
        );
        return log.warn(msg);
      }
